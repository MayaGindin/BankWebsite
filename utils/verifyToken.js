        ...
        if (list['accessToken']) {
            ...
            try {
                ...
                if ((usergroupsAllowed) && (!usergroupsAllowed.includes(user.role))) {
                    return res.render('misc-error', { status: 'error', message: 'You do not have permission to run this request' });
                }
                next();
            } catch (err) {
                return res.render('misc-error', { status: 'error', message: 'Invalid or expired auth token' });
            }
        } else {
            return res.render('misc-error', { status: 'error', message: 'Malformed or unauthenticated auth token' });
        }
